  ...
  return (
    <div>
      ...
      <div className="flex justify-end gap-4 items-center pr-5 pb-5">
        <Input placeholder="Search chapter..." className={"w-[400px]"} />
      ...
      <CustomDataTable
        columns={[
...

          {
            ...
            header: "Status",
            accessorFn: (row) =>
              row.isPublished ? "Plublished" : "Not Plublished",
          },
          {
            accessorKey: "isFree",
            header: "Is Free",
        ...
        data={data?.data || []}
        onRowClick={(data) =>
          navigate(`/app/courses/${data.original.courseId}/${data.original.id}`)
        }
      />
      ...
      <div className="mt-5">
        <PaginationComponent
          ...
          onPageChange={(page) => {
            changeChapterPaginationFn({ page, pageSize: undefined });
          }}
        />
      </div>
      ...
      >
        <AddChapter onClose={() => setChapterFormModalState(false)} />
      </DialogComponent>
    </div>
  );
};
